...
public class IdsMapper {
    ...
    static Ids toDomain(IdsXml x) {
        Ids ids = new Ids();
        if (x.getInfo() != null) {
            ids.getInfo().put("title", x.getInfo().getTitle());
            ids.getInfo().put("description", x.getInfo().getDescription());
            ids.getInfo().put("copyright",   x.getInfo().getCopyright());
            ids.getInfo().put("version",     x.getInfo().getVersion());
            ids.getInfo().put("author",      x.getInfo().getAuthor());
            ids.getInfo().put("date",        x.getInfo().getDate());      // String or LocalDate per your DTO
            ids.getInfo().put("purpose",     x.getInfo().getPurpose());
            ids.getInfo().put("milestone",   x.getInfo().getMilestone());
        }
        return ids;
    }
